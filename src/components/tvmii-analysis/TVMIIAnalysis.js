...
const TVMIIAnalysis = ({ selectedCommodity }) => {
  ...
  const [processedTVMIIData, setProcessedTVMIIData] = useState(null);
  const [processedMarketPairsData, setProcessedMarketPairsData] = useState(null);

  useEffect(() => {
    if (tvmiiData && marketPairsData) {
      try {
        // Filter and process tvmiiData for the selected commodity
        const processedTVMII = tvmiiData.filter(item => item.commodity.toLowerCase() === selectedCommodity.toLowerCase());
        setProcessedTVMIIData(processedTVMII);

        // Filter and process marketPairsData for the selected commodity
        const processedMarketPairs = marketPairsData.filter(item => item.commodity.toLowerCase() === selectedCommodity.toLowerCase());
        setProcessedMarketPairsData(processedMarketPairs);
      } catch (err) {
        setProcessedTVMIIData(null);
        setProcessedMarketPairsData(null);
      }
    }
  ...
  const handleDownloadCsv = () => {
    if (!processedTVMIIData && !processedMarketPairsData) {
      return;
    }
  ...
  return (
    <Paper elevation={3} sx={{ mt: 4, p: { xs: 1, sm: 2 }, width: '100%', backgroundColor: theme.palette.background.paper }}>
      ...
      >
        <Tab label="TV-MII Chart" sx={{ minWidth: isMobile ? 'auto' : 150, fontSize: isMobile ? '0.8rem' : '1rem', textTransform: 'none' }} />
        {processedMarketPairsData && processedMarketPairsData.length > 0 && (
          <Tab label="Market Pairs" sx={{ minWidth: isMobile ? 'auto' : 150, fontSize: isMobile ? '0.8rem' : '1rem', textTransform: 'none' }} />
        )}
        <Tab label="Interpretation" sx={{ minWidth: isMobile ? 'auto' : 150, fontSize: isMobile ? '0.8rem' : '1rem', textTransform: 'none' }} />
      </Tabs>

      <Box sx={{ mt: 2 }}>
        {activeTab === 0 && <TVMIIChart data={processedTVMIIData} selectedCommodity={selectedCommodity} isMobile={isMobile} />}
        {activeTab === 1 && processedMarketPairsData && processedMarketPairsData.length > 0 && (
          <TVMIIMarketPairsChart data={processedMarketPairsData} selectedCommodity={selectedCommodity} isMobile={isMobile} />
        )}
        {activeTab === (processedMarketPairsData && processedMarketPairsData.length > 0 ? 2 : 1) && (
          <TVMIIInterpretation data={processedTVMIIData} marketPairsData={processedMarketPairsData} selectedCommodity={selectedCommodity} />
        )}
      </Box>
      ...
      <Accordion>
        ...
        <AccordionDetails>
          <Typography variant="body1" component="div" sx={{ fontSize: isMobile ? '0.9rem' : '1rem' }}>
            ...
            <ul>
              <li>A TV-MII value close to 1 indicates high market integration, while values close to 0 suggest low integration.</li>
              <li>The TV-MII Chart shows how market integration changes over time for the selected commodity.</li>
              {processedMarketPairsData && processedMarketPairsData.length > 0 && (
                <li>The Market Pairs Chart compares integration levels between different market pairs.</li>
              )}
              <li>Look for trends, sudden changes, or cyclical patterns in the TV-MII values to understand market dynamics.</li>
            </ul>
          </Typography>
        </AccordionDetails>
      </Accordion>
